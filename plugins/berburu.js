...
let handler = async (m, { conn, usedPrefix, owner }) => { 
     try {
        let __timers = (new Date - global.DATABASE.data.users[m.sender].lastberbru)
        let _timers = (300000 - __timers) 
        let timers = clockString(_timers)
        
        if (global.DATABASE.data.users[m.sender].stamina > 60) {
        if (global.DATABASE.data.users[m.sender].healt > 50) {
   ...
   ///// else conn.berbu[m.chat] = true
            if (new Date - global.DATABASE.data.users[m.sender].lastberbru > 300000) {
...
conn.send2Button( m.chat, ber, `Games Rpg`, 'Again', '.berburu', 'inventory', '.inv', m)
                     ...
                     m.reply(`*Mulai Berburu...*`)
                      ...
                      //
                        ...
                        global.DATABASE.data.users[m.sender].stamina -= stamina * 1
                        global.DATABASE.data.users[m.sender].sapi += sapi * 1
                        global.DATABASE.data.users[m.sender].banteng += banteng * 1
               ...
               } else conn.send2Button(m.chat, `Minimal 60 Stamina untuk bisa berburu`, `Games Rpg`, 'Makan', '.ayamb 2','Go back Menu','.gmenu')
                } catch (e) {
        console.log(e)
