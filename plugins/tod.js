...
let handler = async (m, { conn, command, usedPrefix }) => {
  if (/^tod$/i.test(command)) {
    await conn.send2Button(m.chat, 'Truth or Dare', botwm, 'Truth', `${usedPrefix}truth`, 'Dare', `${usedPrefix}dare`, m)
  }
  if (/^truth$/i.test(command)) {
    let res = await fetch(API('pencarikode', '/api/truthid', {}, 'apikey'))
    if (!res.ok) throw eror
    let json = await res.json()
    await conn.send2Button(m.chat, json.message, botwm, 'Truth', `${usedPrefix}truth`, 'Dare', `${usedPrefix}dare`, m)
