...
Bonus: ${poin} XP
    `.trim()
  conn.tebakgambar[id] = [
    await conn.sendButImg(m.chat, await (await fetch(json.img)).buffer(), caption, botwm, 'Bantuan', '.hint', m)
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.tebakgambar[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, botwm, 'Tebak Gambar', '.tebakgambar', conn.tebakgambar[id][0])
